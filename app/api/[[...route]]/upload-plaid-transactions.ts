...
type Transaction2 = {
  ...
  // Add other relevant fields from your Transaction object if needed
};

interface CategorizedResult {
  // Define properties if needed or use `string` if it’s a simple array of categories
  category: string;
}
...
app.get('/fetch-access-token', clerkMiddleware(), async (ctx) => {
...
app.post('/fetch-plaid-transactions', async (ctx) => {
  const { accessToken, startDate, endDate } = await ctx.req.json();

  if (!accessToken || !startDate || !endDate) {
    return ctx.json({ error: "Missing required parameters" }, 400);
  }

  const plaidTransactions = await fetchPlaidTransactionsWithRetry(accessToken, startDate, endDate);

  if (!plaidTransactions) {
    return ctx.json({ error: 'Failed to fetch transactions after retries' }, 500);
  }

  return ctx.json({ transactions: plaidTransactions });
});

app.post('/categorize-transactions', async (ctx) => {
  const { transactions, userId, categoryOptions }: { 
    transactions: Transaction2[]; 
    userId: string; 
    categoryOptions: string[] 
  } = await ctx.req.json();

  if (!transactions || !userId || !categoryOptions) {
    return ctx.json({ error: "Missing required parameters" }, 400);
  }

  const transactionCategories = transactions.map((transaction: Transaction2) => 
  ...
  return ctx.json({ categorizedResults: aiData });
});

app.post('/insert-transactions', async (ctx) => {
  const {
    ...
    userId: string;
    categorizedResults: string[];
    accountIdMap: Record<string, string>;
    dbCategories: Category[];
  } = await ctx.req.json();

  if (!transactionList || !userId || !categorizedResults || !accountIdMap || !dbCategories) {
    return ctx.json({ error: "Missing required parameters" }, 400);
  }

  await Promise.all(
    transactionList.map(async (transaction: Transaction, index: number) => {
