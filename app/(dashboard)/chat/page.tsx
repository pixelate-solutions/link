...
const Chatbot = () => {
  ...
  const [chatHistory, setChatHistory] = useState<Message[]>([
    {
      content: "Hi, how can I help you? I am an AI assistant trained to understand your resources and help you with anything that you need.",
      role: "assistant",
    },
  ]);
  ...
  const handleSendMessage = async () => {
    if (inputMessage.trim() !== "") {
      ...
      if (userId) {
        try {
          const responseStream = await chatAPI(userId, inputMessage);
    ...
    <div className="flex flex-col h-full w-full lg:w-[80%] lg:ml-[10%] -mt-5 lg:-mt-20 pb-[100px]">
      <div className="flex flex-col flex-grow p-4 rounded-2xl bg-white overflow-y-auto">
        <div className="w-full h-[75px] p-2 border-b-2 border-x-2 rounded-xl lg:rounded-b-xl">
          ...
          <div className="flex w-full justify-center">
            <Button onClick={() => setIsAlertDialogOpen(true)} className="bg-transparent text-red-500 rounded-md hover:text-red-400 hover:bg-transparent">
              Clear History
            </Button>
          </div>
      ...
      <div className="p-4 pb-10 fixed bottom-0 rounded-t-2xl full-width-minus bg-white border-t border-gray-300">
        <div className="flex">
          <textarea
            ref={textAreaRef}
            className="flex-grow border border-gray-300 rounded-l-md p-2 resize-none"
