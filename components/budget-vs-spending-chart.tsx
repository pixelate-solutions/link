...
import { format, parseISO, differenceInDays, isSameMonth, getDate, isFirstDayOfMonth, isLastDayOfMonth, endOfToday, subMonths } from 'date-fns';
...
export const BudgetVsSpendingChart = ({ data, fullData, cumulativeBudget }: BudgetVsSpendingChartProps) => {
  ...
  const categoriesWithTotals = categories.map((category: any) => {
    if (category.type === "transfer") {
      ...
      const adjustedBudgetAmount = calculateAdjustedBudget(monthlyBudget, fromDate, toDate);
      
      const foundTotals = totalsQuery.data?.find((t) => t.categoryId === category.id) || { totalIncome: 0, totalCost: 0 };
  ...
  const sumBudgetAmount = categoriesWithTotals.reduce(
    (sum: number, item) => (!item.isTransfer ? sum + parseFloat(item.budgetAmount.toString()) : sum),
    0
  );
  ...
  const processedData = data.map((entry, index) => {
    ...
    const cumulativeBudgetForDay = ((cumulativeBudget || 0) / totalDays) * (index + 1); // Proportional budget
